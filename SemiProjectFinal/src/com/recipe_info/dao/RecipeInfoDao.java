...
public class RecipeInfoDao extends JDBCTemplate {
	...
	public RecipeInfoDto selectrecipeview(int recipe_id) {
		...
		return infodto;
	}public List<RecipeInfoDto> selectcategory(String[] nation_nm,String[] ty_nm,String[] cooking_time) {//
		String resource = "com/recipe/db/recipe-config.xml";
		InputStream inputStream = null;
		try {
			inputStream = Resources.getResourceAsStream(resource);
		...
		SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(inputStream);
		SqlSession session = sqlSessionFactory.openSession();
		List<RecipeInfoDto> infodto=session.selectList("recipe.mapper.selectcategory",map);
		
		//mapper에서 실행하면 냉장고 재료가 가장많이 포함된 recipeid 3개를 찾아서   그 해당 recipe_id의  select* 결과가 3줄쫙옴 그것을 리스트에 담는다.

		return infodto;//recommendrecip;
	}public List<RecipeInfoDto> allrecipe() {//id받고 해당사용자의 냉장고db에 접근해야되기떄문에
		String resource = "com/recipe/db/recipe-config.xml";
		InputStream inputStream = null;
		try {
			inputStream = Resources.getResourceAsStream(resource);
		...
		SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(inputStream);
		SqlSession session = sqlSessionFactory.openSession();
		List<RecipeInfoDto> recommendrecip=session.selectList("recipe.mapper.allrecipe");
		//mapper에서 실행하면 냉장고 재료가 가장많이 포함된 recipeid 3개를 찾아서   그 해당 recipe_id의  select* 결과가 3줄쫙옴 그것을 리스트에 담는다.

		return recommendrecip;
	}  public List <RecipeInfoDto> selectLikeFood(String recipe_nm_ko) {
	      String resource = "com/recipe/db/recipe-config.xml";
	      InputStream inputStream = null;
	      try {
	         inputStream = Resources.getResourceAsStream(resource);
	      ...
	      SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(inputStream);
	      SqlSession session = sqlSessionFactory.openSession();
	      List <RecipeInfoDto> selectLikeFood = session.selectList("recipe.mapper.selectLikeFood",recipe_nm_ko);//결과가 한줄이기떄문에 .selectOne
