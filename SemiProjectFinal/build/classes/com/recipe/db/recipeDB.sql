...
SELECT * FROM RECIPE_INFO WHERE NATION_NM='한식' AND TY_NM='밥' AND COOKING_TIME IN(0,30)
SELECT * FROM RECIPE_INFO WHERE RECIPE_NM_KO='쇠고기양송이볶음' 

SELECT DISTINCT TY_NM FROM RECIPE_INFO;
...
WHERE RECIPE_ID IN 
 ...
 WHERE ID='fake' GROUP BY RECIPE_MATERIAL.RECIPE_ID ORDER BY count DESC) 
 WHERE ROWNUM<=3)
